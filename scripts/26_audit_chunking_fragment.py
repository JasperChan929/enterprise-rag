# scripts/26_audit_chunking_fragment.py
"""Day 8 - 脚本 26 (重写版 v2): 量化 TD-8-1 "财务数据表切断"
...
【预期结果范围】
  全库 6881 chunks 里, 碎片数应该 < 100 (约 1-2%).
  如果 > 500 → 规则太松, 需要重审
...
设计上的节制
===============================================================================
- 不修原脚本, 新建新脚本. 保留原设计作为"反面教材"参考 (面试可用)
- 输出 3 个层次: 全局 / 公司分布 / 样例
...
def is_fragment_chunk(content: str) -> dict:
    """完整判定: 是不是"财务表切断碎片".
    ...
    # 条件 1 不满足 → 不是碎片
    if not kws:
        return {"is_fragment": False, "kws": [], "why": "无财务关键词"}

    # 条件 2 不满足 → 有数字, 不是碎片
    if has_number(content):
        return {"is_fragment": False, "kws": kws, "why": "含数字, 非碎片"}
...
def scan_and_analyze(client):
    ...
    total = 0
    with_financial_kw = 0
    with_kw_no_num = 0        # 满足条件 1 + 条件 2 的 (潜在碎片池)
    fragments = 0             # 三条件全满足的碎片

    by_company = defaultdict(lambda: {
        "total": 0, "with_kw": 0, "with_kw_no_num": 0, "fragment": 0,
    })
    ...
    while True:
        ...
        for p in response:
            ...
            result = is_fragment_chunk(content)
            if result["is_fragment"]:
                fragments += 1
                by_company[key]["fragment"] += 1

                if len(fragment_samples) < 10:  # 存 10 个样例多看几种形态
...
def print_report(result: dict):
    ...
    print("=" * 80)
    print("Day 8 — TD-8-1 财务表切断碎片 chunk 量化 (脚本 26 重写 v2)")
    print("=" * 80)
    ...
    print(f"           └ 这是'潜在碎片池', 含叙述文字")
    print(f"  Level 4: + 表头特征 (条件 1+2+3) = 碎片  {frag}")
    ...
    print(f"\n【按公司_年份分布】 (按碎片数降序)")
    print(f"{'公司_年份':<24} {'总数':>8} {'含关键词':>10} {'潜在':>8} {'碎片':>8} {'碎片率':>10}")
    print("-" * 80)

    sorted_items = sorted(
        result["by_company"].items(),
        key=lambda kv: -kv[1]["fragment"],
    )
    for key, stats in sorted_items:
        ...
        print(f"{key:<24} {t:>8} {w:>10} {p:>8} {f:>8} {rate:>9.1f}%")
    ...
    print(f"  含财务关键词的 chunk 中, 碎片比例 = {frag_rate_in_kw:.1f}% → {sev}")
    print(f"  建议: {rec}")
